...
import { useNewPosts } from "../hooks/post/fecthNewestPost";
...
const NewestFeed = () => {
  ...
  const { data: newPosts, isLoading, error, refetch } = useNewPosts(state.currentPage);
  ...
  useEffect(() => {
    if (newPosts?.posts) {
      setState((prev) => {
        ...
        newPosts.posts.forEach((post: Post) => updatedPosts.set(post._id, post));
        return {
          ...prev,
  ...
  const handleEndReached = useCallback(() => {
    if (state.hasMore && !state.isLoadingMore) {
      setState((prev) => ({ ...prev, isLoadingMore: true, currentPage: prev.currentPage + 1 }));
    }
  }, [state.hasMore, state.isLoadingMore]);

  useEffect(() => {
    if (state.currentPage > 1) {
      refetch().finally(() => setState((prev) => ({ ...prev, isLoadingMore: false })));
    }
